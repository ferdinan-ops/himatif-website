...
export const getBlogsFeatures = () =>
  axiosPublic.get(`${POST_URI}&filters[featured][$eq]=true&pagination[pageSize]=3`)
...
export const getBlogs = (page: number) =>
  axiosPublic.get(`${POST_URI}&pagination[page]=${page}&pagination[pageSize]=6`)
...
export const getBlogByCategoryAndKeyword = (category: string, keyword: string) =>
  axiosPublic.get(`${POST_URI}&filters[judul][$contains]=${keyword}&filters[kategori][slug][$eq]=${category}&${blogPagination}`)

export const getMembers = (year: string) => axiosPublic.get(`/members?populate=photo&filters[tahun_bergabung][$eq]=${year}`)
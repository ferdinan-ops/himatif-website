...
const NavLink = ({ href, children, isTop, isHome }) => {
   ...
   return (
      <Link
         href={href}
         className={`text-[15px] font-medium tracking-wide ${router.pathname === href
            ? "" : isTop
...
const Header = ({ isHome }) => {
   ...
   return (
      <header className={`fixed inset-x-0 top-0 z-50 flex h-24 items-center px-6 transition-all xl:px-0 ${!isTop ? "bg-white text-font-black shadow" : `${!isHome ? "bg-white text-font-black" : "bg-transparent text-font-black xl:text-white"}`}`}>

...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  return { props: { blog: data.data[0], contact: contact.data.attributes.kontak } }
}
...
export default function Blog({ blog, contact }: BlogProps) {
  const { attributes } = blog

  return (
    <Layout title={attributes.judul} contact={contact} isHome={false}>
        ...
        <div className="mx-auto flex flex-col items-center md:w-8/12 xl:w-6/12">
          ...
          <div className="mt-4 flex items-center gap-4 rounded-full px-2 py-1 shadow-lg md:mt-6 md:gap-5 md:px-4 md:py-2">
            {sosialMedia.map((item, index) => (
              <Link key={index} href={item.href} className="rounded-full bg-white p-1.5 text-sm hover:bg-slate-300 md:p-2 md:text-base">
                {item.icon}
              </Link>
            ))}
          </div>
